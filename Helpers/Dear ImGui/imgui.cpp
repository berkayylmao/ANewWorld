    ...
    if (add_to_tab_bar)
    {
        if (node->TabBar == NULL)
        {
            ...
            for (int n = 0; n < node->Windows.Size - 1; n++)
                TabBarAddTab(node->TabBar, ImGuiTabItemFlags_NoCloseButton | ImGuiTabItemFlags_NoCloseWithMiddleMouseButton, node->Windows[n]);
        }
        TabBarAddTab(
            node->TabBar,
...
static void ImGui::DockNodeUpdateTabBar(ImGuiDockNode* node, ImGuiWindow* host_window)
{
    ...
    if (tab_bar->SelectedTabId == NULL && tab_bar->Tabs.Size > 0)
      tab_bar->SelectedTabId = tab_bar->Tabs.front().ID;
    ...
    ImGuiTabBarFlags tab_bar_flags =
        ImGuiTabBarFlags_DockNode | ImGuiTabBarFlags_NoCloseWithMiddleMouseButton;
