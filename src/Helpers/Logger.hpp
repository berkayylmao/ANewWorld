...
#pragma warning(disable : 4505)
  ...
  static void Log(const LogLevel       level, const wchar_t*        message, const wchar_t* const functionName,
  ...
  static void Log(const LogLevel       level, const std::wstring&   message, const wchar_t* const functionName,
                  const wchar_t* const fileName, const std::int32_t lineNumber) {
    Log(level, message.c_str(), functionName, fileName, lineNumber);
  }

  static void Log(const LogLevel       level, const char*           message, const wchar_t* const functionName,
                  const wchar_t* const fileName, const std::int32_t lineNumber) {
    const auto _wsize = MultiByteToWideChar(CP_UTF8, 0, message, std::strlen(message), nullptr, 0);
  ...
  static void Log(const LogLevel       level, const std::string&    message, const wchar_t* const functionName,
                  const wchar_t* const fileName, const std::int32_t lineNumber) {
    Log(level, message.c_str(), functionName, fileName, lineNumber);
  }
...
#define __LOGGER_EXPANDLAYER__(str) L##str
#define __LOGGER_EXPAND__(str)      __LOGGER_EXPANDLAYER__(str)
#define Log(LogLevel_, Message) \
  Logger::Log(LogLevel_, Message, __LOGGER_EXPAND__(__FUNCSIG__), __LOGGER_EXPAND__(__FILE__), __LINE__)
