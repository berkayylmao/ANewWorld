...
#if FMT_USE_NOEXCEPT || FMT_HAS_FEATURE(cxx_noexcept) || (FMT_GCC_VERSION >= 408 && FMT_HAS_GXX_CXX11) || \
    FMT_MSC_VER >= 1900
...
namespace internal {
  ...
  // warnings.
  template <typename T>
  FMT_CONSTEXPR T const_check(T value) {
    return value;
  }
...
#define FMT_USE_INT128 0
  ...
  constexpr bool is_unicode() {
    return FMT_UNICODE || sizeof(Char) != 1 || (sizeof(micro) == 3 && micro[0] == 0xC2 && micro[1] == 0xB5);
  }
...
class basic_string_view {
 ...
 public:
  ...
  FMT_CONSTEXPR basic_string_view(const std::basic_string<Char, Traits, Alloc>& s) FMT_NOEXCEPT : data_(s.data()),
                                                                                                  size_(s.size()) {}
...
inline basic_string_view<Char> to_string_view(const std::basic_string<Char, Traits, Alloc>& s) {
  return s;
}
...
inline basic_string_view<Char> to_string_view(internal::std_string_view<Char> s) {
  return s;
}
...
class basic_format_parse_context : private ErrorHandler {
 ...
 public:
  ...
  explicit FMT_CONSTEXPR basic_format_parse_context(basic_string_view<Char> format_str,
  ...
  /**
    ...
    parsed.
   */
  FMT_CONSTEXPR iterator begin() const FMT_NOEXCEPT { return format_str_.begin(); }
  ...
  FMT_CONSTEXPR void advance_to(iterator it) { format_str_.remove_prefix(internal::to_unsigned(it - begin())); }
...
template <typename Char, typename ErrorHandler = internal::error_handler>
using basic_parse_context FMT_DEPRECATED_ALIAS = basic_format_parse_context<Char, ErrorHandler>;
...
struct FMT_DEPRECATED convert_to_int
    : bool_constant<!std::is_arithmetic<T>::value && std::is_convertible<T, int>::value> {};
...
namespace internal {
  ...
  class buffer {
   ...
   protected:
    ...
    buffer(T* p = nullptr, std::size_t sz = 0, std::size_t cap = 0) FMT_NOEXCEPT : ptr_(p), size_(sz), capacity_(cap) {}
  ...
  class container_buffer : public buffer<typename Container::value_type> {
   ...
   public:
    explicit container_buffer(Container& c) : buffer<typename Container::value_type>(c.size()), container_(c) {}
  };
...
#define FMT_TYPE_CONSTANT(Type, constant) \
  ...
  struct custom_value {
    ...
    const void* value;
    void (*format)(const void* arg, typename Context::parse_context_type& parse_ctx, Context& ctx);
  };
   ...
   public:
    ...
    value(const T& val) {
      ...
      custom.format = format_custom_arg<
          T, conditional_t<has_formatter<T, Context>::value, typename Context::template formatter_type<T>,
  ...
  struct arg_mapper {
    ...
    FMT_CONSTEXPR basic_string_view<char_type> map(const T& val) {
      static_assert(std::is_same<char_type, char_t<T>>::value, "mixing character types is disallowed");
      return to_string_view(val);
    }
    template <typename T,
              FMT_ENABLE_IF(std::is_constructible<basic_string_view<char_type>, T>::value && !is_string<T>::value &&
                            !has_formatter<T, Context>::value && !has_fallback_formatter<T, Context>::value)>
    FMT_CONSTEXPR basic_string_view<char_type> map(const T& val) {
      return basic_string_view<char_type>(val);
    }
    template <typename T,
              FMT_ENABLE_IF(std::is_constructible<std_string_view<char_type>, T>::value &&
                            !std::is_constructible<basic_string_view<char_type>, T>::value && !is_string<T>::value &&
                            !has_formatter<T, Context>::value && !has_fallback_formatter<T, Context>::value)>
    FMT_CONSTEXPR basic_string_view<char_type> map(const T& val) {
    ...
    template <typename T, FMT_ENABLE_IF(std::is_enum<T>::value && !has_formatter<T, Context>::value &&
                                        !has_fallback_formatter<T, Context>::value)>
    FMT_CONSTEXPR auto map(const T& val)
        -> decltype(std::declval<arg_mapper>().map(static_cast<typename std::underlying_type<T>::type>(val))) {
...
class basic_format_arg {
 ...
 public:
  ...
  FMT_CONSTEXPR explicit operator bool() const FMT_NOEXCEPT { return type_ != internal::type::none_type; }
...
FMT_CONSTEXPR auto visit_format_arg(Visitor&& vis, const basic_format_arg<Context>& arg) -> decltype(vis(0)) {
  ...
  switch (arg.type_) {
    ...
    case internal::type::string_type:
      return vis(basic_string_view<char_type>(arg.value_.string.data, arg.value_.string.size));
...
#endif
      data_{internal::make_arg<is_packed, Context, internal::mapped_type_constant<Args, Context>::value>(args)...} {
  }
};
...
#endif
{
 private:
  ...
  template <typename T>
  struct need_copy {
    static constexpr internal::type mapped_type = internal::mapped_type_constant<T, Context>::value;

    enum {
      value = !(internal::is_reference_wrapper<T>::value || std::is_same<T, basic_string_view<char_type>>::value ||
                std::is_same<T, internal::std_string_view<char_type>>::value ||
                (mapped_type != internal::type::cstring_type && mapped_type != internal::type::string_type &&
                 mapped_type != internal::type::custom_type && mapped_type != internal::type::named_arg_type))
    };
  };

  template <typename T>
  using stored_type = conditional_t<internal::is_string<T>::value, std::basic_string<char_type>, T>;
  ...
  unsigned long long get_types() const { return internal::is_unpacked_bit | data_.size(); }
  ...
  */
  ...
  void push_back(const T& arg) {
    static_assert(!std::is_base_of<internal::named_arg_base<char_type>, T>::value,
...
class basic_format_args {
 ...
 public:
  ...
  /**
   ...
   \endrst
   */
  basic_format_args(const dynamic_format_arg_store<Context>& store) : types_(store.get_types()) {
  ...
  format_arg get(int index) const {
    ...
    if (arg.type_ == internal::type::named_arg_type) arg = arg.value_.named_arg->template deserialize<Context>();
    return arg;
  }

  int max_size() const {
    unsigned long long max_packed = internal::max_packed_args;
    return static_cast<int>(is_packed() ? max_packed : types_ & ~internal::is_unpacked_bit);
  }
};
...
struct format_args : basic_format_args<format_context> {
  template <typename... Args>
  format_args(Args&&... args) : basic_format_args<format_context>(static_cast<Args&&>(args)...) {}
};
struct wformat_args : basic_format_args<wformat_context> {
  template <typename... Args>
  wformat_args(Args&&... args) : basic_format_args<wformat_context>(static_cast<Args&&>(args)...) {}
};
...
namespace internal {
  ...
  inline format_arg_store<buffer_context<Char>, remove_reference_t<Args>...> make_args_checked(
      const S& format_str, const remove_reference_t<Args>&... args) {
  ...
  template <typename Char>
  typename buffer_context<Char>::iterator vformat_to(buffer<Char>& buf, basic_string_view<Char> format_str,
                                                     basic_format_args<buffer_context<type_identity_t<Char>>> args);
...
template <typename OutputIt, typename S, typename Char = char_t<S>,
          FMT_ENABLE_IF(internal::is_contiguous_back_insert_iterator<OutputIt>::value)>
...
inline std::back_insert_iterator<Container> format_to(std::back_insert_iterator<Container> out, const S& format_str,
                                                      Args&&... args) {
  return vformat_to(out, to_string_view(format_str), internal::make_args_checked<Args...>(format_str, args...));
}

template <typename S, typename Char = char_t<S>>
inline std::basic_string<Char> vformat(const S&                                                 format_str,
                                       basic_format_args<buffer_context<type_identity_t<Char>>> args) {
...
inline std::basic_string<Char> format(const S& format_str, Args&&... args) {
  return internal::vformat(to_string_view(format_str), internal::make_args_checked<Args...>(format_str, args...));
}
...
inline void print(std::FILE* f, const S& format_str, Args&&... args) {
  return internal::is_unicode<Char>()
             ? vprint(f, to_string_view(format_str), internal::make_args_checked<Args...>(format_str, args...))
...
inline void print(const S& format_str, Args&&... args) {
  return internal::is_unicode<Char>()
             ? vprint(to_string_view(format_str), internal::make_args_checked<Args...>(format_str, args...))
