...
//  2019-05-29: DirectX9: Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset
//  flag. 2019-04-30: DirectX9: Added support for special ImDrawCallback_ResetRenderState callback to reset render
//  state. 2019-03-29: Misc: Fixed erroneous assert in ImGui_ImplDX9_InvalidateDeviceObjects(). 2019-01-16: Misc:
//  Disabled fog before drawing UI's. Fixes issue #2288. 2018-11-30: Misc: Setting up io.BackendRendererName so it can
//  be displayed in the About Window. 2018-06-08: Misc: Extracted imgui_impl_dx9.cpp/.h away from the old combined
//  DX9+Win32 example. 2018-06-08: DirectX9: Use draw_data->DisplayPos and draw_data->DisplaySize to setup projection
//  matrix and clipping rectangle. 2018-05-07: Render: Saving/restoring Transform because they don't seem to be included
//  in the StateBlock. Setting shading mode to Gouraud. 2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback
//  and exposed ImGui_ImplDX9_RenderDrawData() in the .h file so you can call it yourself. 2018-02-06: Misc: Removed
//  call to ImGui::Shutdown() which is not available from 1.60 WIP, user needs to call CreateContext/DestroyContext
//  themselves.
...
static void ImGui_ImplDX9_SetupRenderState(ImDrawData* draw_data) {
  ...
  // (bottom right). DisplayPos is (0,0) for single viewport apps. Being agnostic of whether <d3dx9.h> or
  // <DirectXMath.h> can be used, we aren't relying on D3DXMatrixIdentity()/D3DXMatrixOrthoOffCenterLH() or
...
void ImGui_ImplDX9_RenderDrawData(ImDrawData* draw_data) {
  ...
  // Backup the DX9 transform (DX9 documentation suggests that it is included in the StateBlock but it doesn't appear
  // to)
  ...
  //  2) to avoid repacking vertices: #define IMGUI_OVERRIDE_DRAWVERT_STRUCT_LAYOUT struct ImDrawVert { ImVec2 pos;
  ...
  for (int n = 0; n < draw_data->CmdListsCount; n++) {
    ...
    for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
      ...
      if (pcmd->UserCallback != NULL) {
        ...
        g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, pcmd->VtxOffset + global_vtx_offset, 0,
  ...
  if (global_vtx_offset == 0) g_pd3dDevice->DrawIndexedPrimitive(D3DPT_TRIANGLELIST, 0, 0, 0, 0, 0);
...
bool ImGui_ImplDX9_Init(IDirect3DDevice9* device) {
  ...
  if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) ImGui_ImplDX9_InitPlatformInterface();
...
static void ImGui_ImplDX9_CreateWindow(ImGuiViewport* viewport) {
  ...
  // PlatformHandleRaw should always be a HWND, whereas PlatformHandle might be a higher-level handle (e.g. GLFWWindow*,
  // SDL_Window*). Some back-ends will leave PlatformHandleRaw NULL, in which case we assume PlatformHandle will contain
  // the HWND.
  ...
  HRESULT hr = g_pd3dDevice->CreateAdditionalSwapChain(&data->d3dpp, &data->SwapChain);
  IM_UNUSED(hr);
  IM_ASSERT(hr == D3D_OK);
...
static void ImGui_ImplDX9_DestroyWindow(ImGuiViewport* viewport) {
  // The main viewport (owned by the application) will always have RendererUserData == NULL since we didn't create the
  // data for it.
...
static void ImGui_ImplDX9_ShutdownPlatformInterface() { ImGui::DestroyPlatformWindows(); }

static void ImGui_ImplDX9_CreateDeviceObjectsForPlatformWindows() {
  ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
  for (int i = 1; i < platform_io.Viewports.Size; i++)
    if (!platform_io.Viewports[i]->RendererUserData) ImGui_ImplDX9_CreateWindow(platform_io.Viewports[i]);
}

static void ImGui_ImplDX9_InvalidateDeviceObjectsForPlatformWindows() {
  ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
  for (int i = 1; i < platform_io.Viewports.Size; i++)
    if (platform_io.Viewports[i]->RendererUserData) ImGui_ImplDX9_DestroyWindow(platform_io.Viewports[i]);
}
