...
//  ImGuiConfigFlags_NavEnableGamepad'. [X] Platform: Multi-viewport support (multiple windows). Enable with
...
bool ImGui_ImplWin32_Init(void* hwnd) {
  ...
  // Keyboard mapping. ImGui will use those indices to peek into the io.KeysDown[] array that we will update during the
...
static void ImGui_ImplWin32_UpdateMousePos() {
  ...
  // user) (When multi-viewports are enabled, all imgui positions are same as OS positions)
  ...
  if (HWND focused_hwnd = ::GetForegroundWindow()) {
    ...
    if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable) {
      // Multi-viewport mode: mouse position in OS absolute coordinates (io.MousePos is (0,0) when the mouse is on the
      // upper-left of the primary monitor) This is the position you can get with GetCursorPos(). In theory adding
      // viewport->Pos is also the reverse operation of doing ScreenToClient().
      ...
      // Single viewport mode: mouse position in client window coordinates (io.MousePos is (0,0) when the mouse is on
      // the upper-left corner of the app window.) This is the position you can get with GetCursorPos() +
      // ScreenToClient() or from WM_MOUSEMOVE.
  ...
  // hovering. Important: this information is not easy to provide and many high-level windowing library won't be able to
  // provide it correctly, because
  // - This is _ignoring_ viewports with the ImGuiViewportFlags_NoInputs flag (pass-through windows).
  // - This is _regardless_ of whether another viewport is focused or being dragged from.
  // If ImGuiBackendFlags_HasMouseHoveredViewport is not set by the back-end, imgui will ignore this field and infer the
  // information by relying on the rectangles and last focused time of every viewports it knows about. It will be
  // unaware of foreign windows that may be sitting between or over your windows.
...
// When implementing your own back-end, you can read the io.WantCaptureMouse, io.WantCaptureKeyboard flags to tell if
...
// automatically. If you are trying to implement your own back-end for your own engine, you may ignore that noise.
//---------------------------------------------------------------------------------------------------------
...
static void ImGui_ImplWin32_GetWin32StyleFromViewportFlags(ImGuiViewportFlags flags, DWORD* out_style,
                                                           DWORD* out_ex_style) {
...
static void ImGui_ImplWin32_DestroyWindow(ImGuiViewport* viewport) {
  if (ImGuiViewportDataWin32* data = (ImGuiViewportDataWin32*)viewport->PlatformUserData) {
    if (::GetCapture() == data->Hwnd) {
      // Transfer capture so if we started dragging from a window that later disappears, we'll still receive the MOUSEUP
      // event.
      ::ReleaseCapture();
      ::SetCapture(g_hWnd);
    }
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport* viewport) {
  ...
  if (data->DwStyle != new_style || data->DwExStyle != new_ex_style) {
    ...
    ::SetWindowPos(data->Hwnd, NULL, rect.left, rect.top, rect.right - rect.left, rect.bottom - rect.top,
...
static LRESULT CALLBACK ImGui_ImplWin32_WndProcHandler_PlatformWindow(HWND hWnd, UINT msg, WPARAM wParam,
                                                                      LPARAM lParam) {
