...
namespace Extensions::D3D9::NewLoadingScreens {
  namespace details {
    ...
    struct Backgrounds {
      class BackgroundInfo {
        ...
        void CalculateImageUVs() {
          ...
          if (_displaySize.x > mSize.x && _displaySize.y > mSize.y) {
            ...
            while (_imgLeft >= _displaySizeCenter.x && _imgTop >= _displaySizeCenter.y && (_imgRight + _displaySizeCenter.x) <= mSize.x &&
                   (_imgBottom + _displaySizeCenter.y) <= mSize.y) {
      ...
      explicit Backgrounds() {
        this->mPath          = IOPaths::Directories::GetAssetsDir() / "NewLoadingScreens" / "Backgrounds";
        const auto _infoJson = mPath / "info.json";

        try {
          if (!std::filesystem::exists(_infoJson)) {
            ::MessageBoxA(nullptr, "'info.json' is missing (for 'Backgrounds')!", "Error loading 'Backgrounds'!", MB_ICONERROR | MB_OK);
          }
          else {
            ...
            for (const auto& _info : _doc["Backgrounds"].GetArray()) {
              BackgroundInfo _bgInfo(mPath / _info["FileName"].GetString(),
                                     {static_cast<float>(_info["Width"].GetUint()), static_cast<float>(_info["Height"].GetUint())},
                                     {static_cast<float>(_info["FocalPointX"].GetUint()), static_cast<float>(_info["FocalPointY"].GetUint())});

              this->mBackgrounds.push_back(_bgInfo);
            }
          }
        }
        catch (const std::runtime_error& e) { ::MessageBoxA(nullptr, e.what(), "Error loading 'Backgrounds'!", MB_ICONERROR | MB_OK); }
      }
    };
    ...
    class LoadingTextSprite {
      ...
      void Draw() {
        ...
        ImGui::SetCursorPos({(_displaySize.x * mPivot.x) - (mSize.x - mOffset.x), (_displaySize.y * mPivot.y) - (mSize.y - mOffset.y)});
        ImGui::Image(mSprites[mCurrentFrame], mSize);
      }

      explicit LoadingTextSprite() {
        this->mPath = IOPaths::Directories::GetAssetsDir() / "NewLoadingScreens" /
        ...
        try {
          if (!std::filesystem::exists(_spriteJson)) {
            ::MessageBoxA(nullptr, "Sprite files are missing (for 'LoadingText')!", "Error loading 'LoadingText'!", MB_ICONERROR | MB_OK);
          }
          else {
            rapidjson::Document _doc;
            ...
            this->mSize           =
              ImVec2{static_cast<float>(_doc["Width"].GetUint()), static_cast<float>(_doc["Height"].GetUint())} * _doc["RenderSizeFactor"].GetFloat();
        ...
        catch (const std::runtime_error& e) { ::MessageBoxA(nullptr, e.what(), "Error loading sprite (for 'LoadingText')!", MB_ICONERROR | MB_OK); }
      }
    ...
    static void CreateTextures(LPDIRECT3DDEVICE9 pDevice) {
      ...
      Log(LogLevel::Info, "Creating Backgrounds textures...");
      {
        ...
        for (auto& background : backgrounds.mBackgrounds) {
          ...
          if (FAILED(_result = D3DXCreateTextureFromFileExW(pDevice,
            background.mPath.wstring().c_str(),
            static_cast<UINT>(background.mSize.x),
            static_cast<UINT>(background.mSize.y),
            1,
            0,
            D3DFMT_UNKNOWN,
            D3DPOOL_MANAGED,
      ...
      Log(LogLevel::Info, "Creating LoadingText texture...");
      {
        ...
        for (std::size_t i = 0; i < loadingTextSprite.mTotalFrames; i++) {
          ...
          if (FAILED(_result = D3DXCreateTextureFromFileExW(pDevice,
            _file.wstring().c_str(),
            static_cast<UINT>(loadingTextSprite.mSize.x),
            static_cast<UINT>(loadingTextSprite.mSize.y),
            1,
            0,
            D3DFMT_UNKNOWN,
            D3DPOOL_MANAGED,
  ...
  static void Draw(LPDIRECT3DDEVICE9 pDevice) {
    ...
    if (!details::texturesCreated) {
      if (!details::alreadyCreatingTextures) auto _dummy = std::async(std::launch::async, details::CreateTextures, pDevice);
        ...
        // Background
        {
          ...
          if (_backgroundSecondsLeft <= 0.0f) {
            ...
            details::backgrounds.mBackgrounds[_nextBackgroundImageIdx].Draw(
              {1.0f, 1.0f, 1.0f, (_backgroundTransitionDuration - _backgroundTransitionSecondsLeft) / _backgroundTransitionDuration});
  ...
  static void Init() {
    ...
    {
      MemoryEditor::Get().Make(MemoryEditor::MakeType::Call,
                               MemoryEditor::Get().AbsRVA(0x897A6),
                               reinterpret_cast<std::uintptr_t>(&details::Hooks::hkLoadingScreen_LoadScreen));
      MemoryEditor::Get().Make(MemoryEditor::MakeType::Jump,
                               MemoryEditor::Get().AbsRVA(0x896FE),
